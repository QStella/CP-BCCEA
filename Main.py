        ...
        for Proc in ProcData:
            ...
            for algorithm_num in range(Algorithm_Number):
                ...
                for gen in range(Gen):
                    ...
                    New_Chromo = np.vstack((Chromo, CR_Chromo))
                    New_Chromo_EDA = np.vstack((New_Chromo, np.array(EDA_Population)))

                    Fit = Fitness(New_Chromo_EDA, N, S, STMach, LotQ, ProcessTime)
                    Se_Chromo = selection(New_Chromo_EDA, Pop_Size, Fit)  # 选择 (最终选取 Pop_Size 个)
                ...
                Path_Result_BCCEAP = OS_Path + "/Result/" + Str_Name + "/BCCEAP"
